...
def barrett_32(regIn, reg_tmp, reg_qR2inv, reg_q, regOut):
	print('  smmulr.w %s, %s, %s' % (reg_tmp, reg_qR2inv, regIn))
	print('  mls.w %s, %s, %s, %s' % (regOut, reg_q, reg_tmp, regIn))


def barrett_32x2(regInL, regInH, reg_tmp, reg_qR2inv, reg_q, regOut):
	barrett_32(regInL, reg_tmp, reg_qR2inv, reg_q, regInL)
	barrett_32(regInH, reg_tmp, reg_qR2inv, reg_q, regInH)
...
def print_Karatsuba():
	...
	if LVL:
		...
		copy_input_coefs(the_other_input)
		extend_input_coefs(the_other_input)
		copy_input_coefs('c')
...
def extend_input_coefs(arr_name):
	...
	for lvl_id in range(LVL):
		...
		if arr_name != 'f':
			for rid in range(MOVE): barrett_16x2(tmp_reg[rid], tmp_reg[MOVE], tmp_reg[MOVE + 1], 'r3', 'r2', 'r1')
		...
		count_bndry //= 2
	if arr_name == 'd': print('  vmov.w r1, s2')
...
def hybrid_mult():
	...
	if (no_small_input == 'y') or (LVL == 0 and output_mode == 't'):
		print('  movw.w r3, #%d' % (qR2inv % (2 ** 16)))
		print('  movt.w r3, #%d' % (qR2inv // (2 ** 16)))
		print('  vmov.w s2, r3')
	print('  ldr.w r4, [r1, #4]')
			...
			if ((product_idx < B // 4 - 1) and (product_idx % 2)) or ((product_idx >= B // 4 - 1) and (1 - product_idx % 2)):
				...
				if LVL == 0 and output_mode == 't':
					...
					if (product_idx == B // 2 - 2):
						...
						barrett_32(str_reg[6], 'r3', 'r6', 'r5', str_reg[3])
						print('  ubfx.w %s, %s, #0, #16' % (str_reg[3], str_reg[3]))
				else:
					num_str_reg_released = 7 if (product_idx == B // 2 - 2) else 4
					for rid in range(num_str_reg_released): barrett_32(str_reg[rid], 'r3', 'r6', 'r5', str_reg[rid])
			else:
				print('  vmov.w s3, r5')
				print('  vmov.w r4, s2')
				print('  movw.w r3, #%d' % (q))
				if LVL == 0 and output_mode == 't':
					...
					for rid in range(4): barrett_32(str_reg[rid], 'r5', 'r4', 'r3', str_reg[rid])
				print('  vmov.w r5, s3')
		if (product_idx == B // 2 - 2):
